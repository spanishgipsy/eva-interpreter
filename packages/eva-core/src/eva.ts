// very strange practise to use .js to make ESM modules available to be imported in dist/* code
import GLOBAL_ENVIRONMENT from './global-environment.js';
import EvaTransformer from './transform/eva-transformer.js';
import EvaModuleLoader from "./modules/eva-module-loader.js";
...
export default class Eva {
  ...
  private readonly moduleLoader: EvaModuleLoader;

  constructor(
    private readonly host: EvaHost = new EvaNoopHost(),
    private readonly global = GLOBAL_ENVIRONMENT,
  ) {
    this.moduleLoader = new EvaModuleLoader(this.host.fs);
  }

  eval(exp: EvaCode, options: EvaEvalOptions = {}): EvaResultValue {
    if (!options.moduleId) {
      throw new TypeError("moduleId is required for eval()");
    }
    this.global.setModuleId(options.moduleId);
    return this.evaluate(exp, this.global);
  }

  private evaluate(exp: EvaCode, env: EvaEnvironment): EvaResultValue {
    ...
    if (Array.isArray(exp)) {
      ...
      const fn = this.evaluate(statement, env) as EvaLambdaFunction | Function;
      const args = exp.slice(1).map(arg => this.evaluate(arg, env));
  ...
  private declareVariable([, name, value]: EvaVariableDeclare, env: EvaEnvironment): EvaResultValue {
    return env.define(name, this.evaluate(value, env));
  }
  ...
  private assignVariable([, ref, value]: EvaVariableAssign, env: EvaEnvironment): EvaResultValue {
    if (ref[0] === EvaStatement.prop) {
      ...
      const instanceEnv = this.evaluate(instanceName, env) as EvaEnvironment;
      return instanceEnv.define(propName, this.evaluate(value, env));
    }
    return env.assign(ref as string, this.evaluate(value, env));
  }
  ...
  private switch(exp: EvaSwitch, env: EvaEnvironment): EvaResultValue {
    ...
    return this.evaluate(ifExp, env);
  }
  ...
  private def(exp: EvaDef, env : EvaEnvironment) {
    ...
    return this.declareVariable(varExp, env);
  }
  ...
  private class([, name, parentName, body]: EvaClass, env : EvaEnvironment): EvaEnvironment {
    const parentEnv = this.evaluate(parentName, env) as EvaEnvironment || env;
  ...
  private new([, className, ...params]: EvaNew, env : EvaEnvironment): EvaEnvironment {
    const classEnv = this.evaluate(className, env) as EvaEnvironment;
    const instanceEnv = new EvaEnvironment({}, classEnv);
    const args = params.map(arg => this.evaluate(arg, env));
  ...
  private prop([, instance, name]: EvaProp, env: EvaEnvironment): EvaResultValue {
    const instanceEnv = this.evaluate(instance, env) as EvaEnvironment;
  ...
  private module([, name, body]: EvaModule, env: EvaEnvironment, moduleId: EvaModuleId = null): EvaEnvironment {
    const moduleEnv = new EvaEnvironment({}, env);
    if (moduleId) {
      moduleEnv.setModuleId(moduleId);
    }
    this.evalBlock(body, moduleEnv);
    return env.define(name, moduleEnv);
  }

  private import([, name, fileName]: EvaImport, env: EvaEnvironment): EvaEnvironment {
    const moduleId = this.host.resolver.resolve(fileName ?? name, env.getModuleId());
    const moduleBody = this.moduleLoader.getOrLoadModule(moduleId);
    const module: EvaModule = [EvaStatement.module, name, moduleBody];
    return this.module(module, this.global, moduleId) as EvaEnvironment;
  }

  private inc(exp: EvaInc, env: EvaEnvironment): EvaResultValue {
    const setIncExp = this.transformer.transformIncToSetPlus(exp);
    return this.evaluate(setIncExp, env);
  }

  private dec(exp: EvaDec, env: EvaEnvironment): EvaResultValue {
    const setDecExp = this.transformer.transformDecToSetMinus(exp);
    return this.evaluate(setDecExp, env);
  }
  ...
  private evalBlock(block: EvaCode[], context: EvaEnvironment): EvaResultValue {
    let result: EvaResultValue = null;
    const [, ...expressions] = block;
    expressions.forEach(exp => result = this.evaluate(exp, context));
    return result;
  }
